...
const Container2 = ({ className = "" }) => {
  ...
  const contentData = {
    mainHeading: "User Feedback",
    reviews: [
      {
        description: "TestApp is incredibly efficient and easy to use. The prompt response feature is impressive.",
        subDescription: "Efficient and user-friendly!",
      },
      {
        description: "The name-to-last-name conversion tool is a unique and useful addition. Very innovative.",
        subDescription: "Innovative features!",
      },
      {
        description: "Extracting text from PDFs with TestApp has streamlined our workflow significantly.",
        subDescription: "Streamlined workflow!",
      },
    ],
  };
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-end justify-start gap-[28px] min-w-[542px] max-w-full text-[20px] mq900:min-w-full">
        {contentData.reviews.map((review, index) => (
          <div key={index} className="self-stretch bg-white box-border overflow-hidden flex flex-col items-start justify-start pt-[42px] pb-[29px] pr-5 pl-[46px] gap-[21px] max-w-full border-[1px] border-solid border-black">
